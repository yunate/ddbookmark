...
export class BookmarkController {
  ...
  private async jumpTo(bookmark: BookmarkItem) {
    ...
    let status = await utils.fileStatus(bookmark.filePath!, lineNumber);
    if (status === utils.FileExistsStatus.FileNotExist) {
      utils.showTip("Error: File not exist");
      return;
    } else if (status === utils.FileExistsStatus.LineNotExist) {
      utils.showTip("Warning: Line not exist");
      lineNumber = 1;
    }
    ...
    vscode.window.showTextDocument(path, {
      ...
      reject => {
        utils.showTip("Error: Failed to navigate to bookmark: " +
          reject.message);
      }
    );
  }
